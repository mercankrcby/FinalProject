 ...
 */
public class AbroadToBondedDomesticFlights extends FlightType{
    public GatesAndOperationCenters goc;
    public ArrayList<GatesAndOperationCenters> gocList;
    public AbroadToBondedDomesticFlights(int arrivalGateNumber,int departureGateNumber) {
    ...
    public void Operation()
    {
        ...
        for(int i=0;i<gocList.size();++i)
        {
            if(gocList.get(i).gateNumber==gateNumberDeparture && gocList.get(i).operationCenterName=="PasaportControlCenter")
            {
                distance+=gocList.get(i).distance;
            }
        }
